---
title: "Crop Spatial Variability and Site-Specific Crop Management"
...
- Overview of site-specific nutrient management (SSNM) principles
- Integration of inputs: climate, soil, machinery, labor, agrochemicals
- Precision-farming cycle: inventory, GIS analysis, GNSS implementation, monitoring, feedback
- 4R framework: right product, rate, place, time
- Spatial interpolation techniques: IDW, kriging, cokriging with terrain data
- Environmental drivers shaping SSNM design: terrain, climate, soil, water

::: {.notes}
This slide outlines the presentation structure and introduces core SSNM concepts. We begin with an overview of SSNM principles, emphasizing the need to tailor fertilizer applications to spatial and temporal crop demand. Next, we discuss the integration of diverse farm management inputs—climatic data, soil testing, machinery capabilities, labor availability, and agrochemical characteristics—and how they feed into prescription models. The precision-farming cycle mirrors the Plan-Do-Check-Act model: first we inventory site characteristics, then perform GIS-based spatial analysis to delineate management zones, implement plans via GNSS-guided equipment, monitor outcomes through yield and soil sensors, and finally incorporate feedback for continuous improvement. We then introduce the “4R” framework—right product, rate, place, and time—as a decision support tool for nutrient stewardship. Advanced spatial interpolation methods such as inverse-distance weighting, ordinary kriging, and cokriging with high-resolution terrain covariates are highlighted for generating accurate fertilizer recommendation maps. Finally, we touch on key environmental drivers—terrain slope and aspect, climatic regime, soil texture and chemistry, and plant-available water—that influence SSNM system design and performance. This agenda sets the stage for in-depth exploration of SSNM methodologies, case studies, and remaining research needs.
:::
...
- Closed-loop control of irrigation and nutrient delivery systems
...
## Abiotic Factors: Soil & Weather

- Apparent soil electrical conductivity (ECa) integrates multiple soil properties into a single field-scale metric
- ECa maps spatial variability of salinity, texture, moisture, organic matter, and compaction
- Common measurement methods: EM induction (horizontal/vertical), resistivity, lab extract conductivities
- Geo-referenced ECa surveys via GPS, integrated into GIS for spatial analysis
- Delineation of site-specific management zones to guide variable-rate irrigation and fertilization
- Supports monitoring of spatio-temporal soil and crop variability for sustainable farming

::: {.notes}
Apparent soil electrical conductivity (ECa) is a versatile abiotic indicator in precision agriculture. It simultaneously reflects soluble salts, clay content and mineralogy, water content, bulk density, organic matter, and temperature, providing a composite view of edaphic conditions. Traditional applications targeted salinity assessment in arid, irrigated zones, but modern surveys leverage ECa to map texture variations, moisture gradients, depth to clay layers, flood-deposited sands, and anthropogenic factors like compaction and leaching patterns. Measurement approaches include electromagnetic induction in horizontal (EMh) and vertical (EMv) coil configurations, electrical resistivity profiling, and laboratory extract methods (ECe, EC1:1). When geo-referenced using GPS and integrated within GIS platforms, ECa datasets enable delineation of management zones for variable-rate irrigation, fertilization, and other inputs. This high-resolution soil mapping underpins sustainable, site-targeted farming, enhancing yield stability, resource use efficiency, and environmental stewardship.
:::
...
- Soil-based data: sampling (texture, fertility, depth), proximal sensing (EC, resistivity, reflectance), RTK-GPS elevation mapping
- Crop-based metrics: yield, quality, plant-tissue tests, NDVI and other vegetation indices via satellite/aerial imagery and ground sensors
- Integration of static soil maps with in-season remote sensing to capture temporal yield variability
- Management zone typologies:
  - Irrigation zones (soil water content, EC, yield data)
  - Nutrient zones (variable N based on EC, elevation, depth)
  - Pesticide/herbicide zones
- Case study – citrus orchards: delineated MZs using yield, canopy volume, NDVI, EC, elevation; canopy volume best correlated with yield (r = 0.85)
- Bayesian spatio-temporal modeling for N optimization: integrates field experiments, market prices, input costs, and climate variability

```{mermaid}
graph TD
    A[Soil Data] --> C[Integration]
    B[Crop Data] --> C
    C --> D[Define MZs]
...
::: {.notes}
In this slide, we delve into the practical methodologies for delineating and managing precision agriculture zones. First, we review soil-based data sources. Soil sampling provides static measures of texture, fertility, and depth, while proximal sensors measure electrical conductivity, resistivity, and reflectance. RTK-GPS enabled elevation mapping further enriches the spatial dataset.

Next, crop-based metrics such as yield maps, quality assessments, plant-tissue analyses, and vegetation indices (e.g., NDVI) are collected via satellite, aerial platforms, or ground sensors. These dynamic indicators capture crop health and biomass changes over the season. However, relying on soil data alone can fail to account for temporal variability in yield.

Therefore, best practices integrate static soil property maps with in-season remote sensing to better reflect changing crop conditions. This integrated dataset underpins the delineation of different management zone typologies: irrigation zones tailored by soil water content, EC, and yield; nutrient zones optimized for variable-rate nitrogen applications based on EC, elevation, and soil depth; and pesticide/herbicide zones defined by pest pressure and canopy characteristics.

We highlight a citrus orchard case study where MZs were delineated using multi-source data: yield, canopy volume, NDVI, EC, and elevation. Canopy volume demonstrated the strongest statistical correlation with yield (r = 0.85), underscoring its predictive power. Finally, we discuss Bayesian spatio-temporal models that optimize nitrogen management by integrating experimental results, commodity prices, input costs, and climate variability, thereby providing a probabilistic framework for decision support.
:::

---

## Biochemical & Biophysical Properties: Overview
...
```{mermaid}
graph LR
  A[Data Acquisition: Satellite & UAV] --> B[Preprocessing: Atmospheric Correction]
  ...
  E --> G[LAI Map]
  F --> G[LAI Map]
  G --> H[Precision Application: VRI, VRF]
```
...
## In-Field Sensors for Chlorophyll

- Noncontact EM38 and contact Veris 3100 sensors for in-field soil ECa mapping as proxies for chlorophyll variability.
- Depth-specific ECa measurements: shallow (0–30 cm) and deep (0–100 cm) profiles.
- Strong correlation (r = 0.74–0.88) between EM38 vertical-mode and deep Veris ECa across diverse fields.
- ECa exhibits robust correlations with clay content and CEC, key factors affecting nutrient availability for chlorophyll synthesis.
- Variable relationships with organic carbon and silt; limited (< 10 %) correlation with sand, moisture, and paste EC.
- Similar spatial ECa patterns from both sensors enable delineation of zones for targeted chlorophyll monitoring.

::: {.notes}
In precision agriculture, in-field sensors like EM38 and Veris 3100 measure apparent soil electrical conductivity (ECa) to infer soil texture, moisture, bulk density, salinity, and CEC variability. This study deployed both sensors across two Illinois and two Missouri fields, collecting georeferenced ECa readings alongside paired 120-cm soil cores at 12–21 locations per field. ECa data were depth-weighted using published response curves for shallow (0–30 cm) and deep (0–100 cm) horizons.

Results showed a high correlation (r = 0.74–0.88) between EM38 vertical-mode and deep Veris measurements, although stratified layers in Missouri introduced divergence. ECa correlated most consistently with clay content and CEC, which are critical for nutrient retention and thus chlorophyll synthesis. Organic carbon and silt also showed field-specific significance, while sand, moisture, and paste EC correlated in fewer than 10 % of cases. Both sensors produced similar spatial patterns, enabling the delineation of management zones for targeted chlorophyll monitoring and nutrient application.

Transition: Next, we will explore drone- and satellite-based remote sensing techniques for direct chlorophyll quantification.
:::
...
- Linking spectral indices to EM-derived soil moisture and texture layers
...
- Integration into 3D predictive models for zone-specific irrigation and fertilization
...
- Hyperspectral detection of nutrient deficiencies and water stress
- Sensor calibration and radiometric correction workflows
- UAV and satellite integration for multi-scale spectral mapping
...
- Penetrates canopy to reveal soil moisture and composition variations
- Distinct crop-specific SWIR signatures enable accurate species differentiation
- Enhanced management zone delineation when combined with clustering algorithms
- Improves intra-zone homogeneity assessment and reduces within-zone yield variance



*Overview of SWIR spectral characteristics and their applications in precision agri-clustering.*
...
- In Smart-Map, NDVI/EVI rasters pass Moran’s I filtering and serve as SVM covariates
...
*Figure: Quality data to inform decisions*
...
- High-resolution multi-spectral imagery (Sentinel-2, Landsat 8) for calculating vegetation indices (NDVI, EVI)
- Temporal monitoring of crop growth stages and early stress detection via time-series analysis
- Integration with Smart-Map plugin: raster bands used as SVM covariates for enhanced interpolation
- Generation of variable-rate application maps by combining satellite-derived moisture and chlorophyll metrics
- Validation against ground-truth soil and yield data, reporting R² (0.05–0.83) and RMSE (0.07–12.01)
- Scalable workflow: imagery ingestion, preprocessing, model fitting, and export in QGIS GUI

```{mermaid}
flowchart LR
    A[Satellite Imagery] --> B[Preprocessing & Index Calculation]
    B --> C[Smart-Map SVM Interpolation]
    C --> D[Continuous Soil Property Maps]
    D --> E[Variable-Rate Application Zones]
```

*Figure: Illustrates the workflow and benefits of integrating satellite imagery with Smart-Map’s SVM interpolation for precision agriculture.*

::: {.notes}
Satellite imagery provides an indispensable data layer for precision agriculture by capturing spatially continuous information on crop and soil conditions. High-resolution sensors like Sentinel-2 and Landsat 8 enable computation of vegetation indices (e.g., NDVI, EVI) which correlate strongly with biomass, canopy cover, and chlorophyll content. Time-series analysis of these indices allows agronomists to detect stress events—such as water deficit, nutrient shortage, or disease onset—well before they manifest visibly in the field. By feeding multi-spectral bands and derived indices into the Smart-Map plugin’s SVM interpolator, we harness machine-learning to produce continuous soil property maps with higher fidelity compared to geostatistical methods alone. The output maps, validated using R² and RMSE metrics against field samples, underpin variable-rate fertilizer and irrigation prescriptions. This slide summarizes the satellite imagery workflow from acquisition and preprocessing to model integration and management zone delineation, demonstrating its technical rigor and practical value for site-specific input optimization.
:::
...
- Spatial heterogeneity in pH, texture, organic matter, moisture, CEC and nutrient concentrations
- Proximal sensing technologies: electromagnetic induction, gamma-ray spectrometry, VNIR spectroscopy
- Remote imagery integration: multispectral indices, UAV-based mapping for surface parameter estimation
- Digital Soil Mapping using covariates (DEM, climate, land use) and geostatistical models (kriging, random forest)
- Variogram analysis to set sampling intervals and delineate management zones at field scale
- Fusion of soil maps with yield and crop sensor data to create functional management units

::: {.notes}
Soil property variability underpins precision agriculture by defining management units that optimize input use and environmental outcomes. High-resolution mapping of soil pH, texture, organic matter and moisture is achieved through proximal sensors (electromagnetic induction, gamma-ray, VNIR spectroscopy) complemented by remote imagery from satellites and UAVs. Digital Soil Mapping (DSM) leverages environmental covariates and geostatistical techniques such as kriging and machine-learning models (e.g., random forest) to predict continuous soil property surfaces (McBratney et al., 2003). Variogram analysis guides sampling density and delineation of management zones that balance within-zone homogeneity with operational feasibility. Finally, integrating these soil datasets with yield maps and canopy sensor outputs produces functional units that align strategic, tactical and operational decision-making, improving economic returns and reducing environmental impact.
:::
...
## Management Practice Variability

- Spatial variability: field-scale differences in soil and yield metrics (e.g., zones yielding 1 t/ha vs. 4 t/ha).
- Temporal variability: seasonal or inter-annual shifts at fixed locations (e.g., yield drop from 2.5 t/ha to 1.5 t/ha).
- Zone delineation: defining contiguous management zones based on variability magnitude and patterns for SSCM.
- Multi-scale assessment: variability analysis from within-field to whole-farm scales using repeated ground and remote sensing measurements.
- Soil texture: sand–silt–clay–gravel proportions control water-holding, nutrient dynamics, chemical binding; mapped via soil EC and gamma radiometric surveys.
- Soil structure: pore architecture influences root penetration, aeration, drainage and erosion risk; degrades under compaction, tillage and organic-matter loss.

::: {.notes}
Understanding farm variability is central to precision agriculture. Spatial heterogeneity emerges from inherent soil–landscape interactions: texture gradients alter moisture retention and nutrient supply, while structure variability modulates root access and aeration. These zones can be mapped via geophysical proxies such as electrical conductivity and gamma radiometrics, guiding site-specific sampling and intervention. Temporal variability, driven by seasonal weather and management legacies, necessitates repeated surveys between growing seasons to capture dynamic changes in yield potential. By delineating management zones that are both statistically distinct and agronomically meaningful, practitioners can target fertilizer, irrigation and tillage to areas of greatest return. Research shows that well-defined, contiguous zones simplify logistical implementation and maximize the economic and environmental benefits of SSCM. Integrating multi-scale data—from within-row sensors to farm-wide remote imaging—provides the resolution needed to link soil drivers (texture, structure) to crop response, enabling adaptive management across seasons.
:::
...
- Continuous soil electrical conductivity (EC) & moisture logging via in-ground sensors at 30-minute intervals
- RTK-GNSS geo-tagging for sub-decimeter spatial accuracy of sensor points
- Weekly in-season NDVI acquisition from UAV and satellite platforms for canopy chlorophyll mapping
- Edge data loggers with local buffering & automated cloud synchronization to GIS database
- Automated QC workflows: outlier detection, temporal gap filling, geostatistical interpolation
...
- Farm baseline: 500 ha mixed cropping under 350 mm rainfall zone
- Profit drivers mapped to PA pathways: yield variability, input costs & risk reduction
- Tool assessment: soil & yield mapping, remote sensing & variable-rate application
- ROI modelling: projected +12 % net profit via site-specific nutrient management
- Data workflow: capture → processing → prescription → machinery integration
- Implementation & monitoring: iterative validation with ROI tracking

::: {.notes}
This integrated case study demonstrates the end-to-end application of the GRDC five-step precision agriculture framework on a representative southern Australian mixed cropping farm. We begin by establishing a farm baseline (500 ha, 350 mm RFE) and quantifying key profit drivers—yield variability, input costs and production risks.

Next, we assess fit-for-purpose PA tools including soil and yield mapping, drone and satellite remote sensing, and variable-rate fertiliser applicators to address identified profit gaps.

ROI modelling leverages cost and benefit projections, indicating a conservative 12 % increase in net profit through precision nutrient placement. A robust data workflow is designed: geo-referenced data capture (soil, yield and canopy indices), cloud-based processing for zone delineation, prescription generation and direct machinery integration. Implementation is staged with iterative validation—ground truthing yields, refining algorithms and tracking ROI metrics against baseline to ensure continuous improvement. This case underscores the necessity of aligning technology with profit objectives and embedding feedback loops for sustained PA success.
:::
...
- Profit-First Framework: five critical questions, Profit Drivers Framework, and Profit Pathways
- Decision Steps: farm-specific data selection, PA technology assessment, system integration, ROI modelling, implementation planning
- Variable-Rate Technologies: seeding, fertiliser, and input optimization linked to yield response curves
- Technical Foundations: VRT, NDVI, GPS geo-referencing, satellite & sensor data integration
- Hardware & Software Checklists: data capture, storage, analysis pipelines, equipment calibration standards
- Documented ROI: empirical case studies showing 10–20% profit uplift from PA deployments

::: {.notes}
This slide distills the core components of the step-by-step Precision Agriculture decision guide. We reaffirm the Profit-First Framework, which employs five critical questions and the Profit Drivers Framework to quantify yield, cost, and risk levers, culminating in Profit Pathways that map variable-rate interventions to dollar gains. The Decision Steps segment highlights sequential activities—from identifying farm-specific opportunities with combined financial, agronomic, and spatial data to assessing technologies (soil mapping, yield mapping, satellite and sensor data) and integrating systems for seamless workflow. Technical Foundations cover essential concepts (VRT, NDVI, geo-referencing) and best practices for data management and analytics, ensuring growers understand both theory and practical implementation. Our checklists for hardware, software, and services standardize requirements for data capture, storage, and field application, reducing deployment risk. Finally, we underscore documented ROI from R&D trials and on-farm case studies, which demonstrate 10–20% profit improvements, validating the structured, profit-focused approach. Growers and advisers can leverage these insights to make evidence-based investments and realize tangible returns through PA adoption.
:::
...
- Clarifications on SSNM system components and data inputs
- Discussion on the precision-farming Plan-Do-Check-Act cycle
- Soil sampling strategies: grid vs. geostatistical methods
- Variations in SSNM decision algorithms across climates
- Impacts on nutrient-use efficiency and sustainability
- Future research directions and technological challenges

::: {.notes}
This slide opens the floor for in-depth discussion of the Site-Specific Nutrient Management (SSNM) framework. Encourage participants to ask about how environmental inputs such as precipitation, temperature, and terrain are integrated with proximal or remote sensing in GIS-based zone delineation. Be ready to explain the iterative Plan-Do-Check-Act cycle and its four rights (right product, rate, place, time). Provide insights on soil sampling methodologies—comparing early 4-ha grid sampling with modern interpolation techniques like inverse-distance weighting, kriging, and cokriging—and their impact on map accuracy. Discuss how SSNM algorithms differ in sub-humid versus semi-arid regions and the resulting effects on nutrient-use efficiency, yield, economic return, and environmental outcomes. Finally, invite suggestions on cutting-edge sensors, machine learning approaches, and decision-support tools that could advance SSNM research and practice.
:::
